...
JobPoolPerf::TaskData::TaskData(std::function<void()> workFn, std::function<void()> completionFn)
    : WorkFn(workFn)
...
JobPoolPerf::JobPoolPerf(size_t maxThreads)
{
    ...
    {
        _threads.emplace_back(&JobPoolPerf::ProcessQueue, this);
    }
}
...
void JobPoolPerf::AddTask(std::function<void()> workFn, std::function<void()> completionFn)
{
    unique_lock lock(_mutex);
...
void JobPoolPerf::Join(std::function<void()> reportFn)
{
    unique_lock lock(_mutex);
    while (true)
    {
...
size_t JobPoolPerf::CountPending()
{
    unique_lock lock(_mutex);
    return _pending.size();
}

void JobPoolPerf::ProcessQueue()
{
    unique_lock lock(_mutex);
    do
    {
